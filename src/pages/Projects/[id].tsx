...
export default function ProjectDetails() {
  ...
  const confirmRemoveMember = async (member: string) => {
    if (!project) return;
      ...
      {confirmState ? (
        ...
        >
          ...
          >
            <div className={headerStyles.modalHeader}>
              <div>
                <p className={headerStyles.modalKicker}>Please confirm</p>
                <h3 className={headerStyles.modalTitle}>
                  {confirmState.type === "delete"
                    ? "Delete this project?"
                    : `Remove ${confirmState.member}?`}
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className={headerStyles.form}>
              <p className={styles.noticeBox}>
                {confirmState.type === "delete"
                  ? "This action cannot be undone and will permanently delete the project."
                  : "This member will lose access until they rejoin."}
              </p>
                ...
                <button
                  ...
                  onClick={() => {
                    if (confirmState.type === "delete") {
                      ...
                      void confirmRemoveMember(confirmState.member);
                    }
                    setConfirmState(null);
                  }}
                  disabled={isLoading}
                >
                  {confirmState.type === "delete" ? "Delete" : "Remove"}
                </button>
              </div>
            </div>
          </div>
